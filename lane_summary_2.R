...
##
##R CMD BATCH -Tu_0110 -1.2.3 -4 -5 /data2/pipeline_in/Runs/useful_R/lane_summary_2.R
...
png_name=paste(file_prefix,"counts_by_sample_by_lane.png",sep="_")
png(png_name)
boxplot(read_count~Lane,data=df_files2,cex=0.5, main="Read counts by sample and lane, colored by sample", ylab="Counts")
beeswarm(read_count~Lane,data=df_files2,pwcol=df_files$sample, pch=18, cex=1.3, add=TRUE)
legend('topright', lane_sums, cex=0.7)
mtext(side=1,text=file_prefix,line=3,cex=1)
dev.off()

summary_table<-df_files2
summary_table<-summary_table[summary_table$Read=="R1",]
csv_name=paste(file_prefix,"counts_by_sample_by_lane.csv",sep="_")
